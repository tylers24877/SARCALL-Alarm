...
object DialogSignOff {
    ...
    fun open(context: Context, view: View, binding: FragmentRespondBinding ) {
        ...
        if (teamPrefixArray.isNullOrEmpty()) {
            Snackbar.make(binding.respondConstraintLayout, context.getString(R.string.fragment_respond_dialog_sign_off_no_team_prefix_toast), Snackbar.LENGTH_LONG).show()
            return
        }
        if (teamPrefixArray.size > 1) {
            ...
            dialogBinding.apply {
                ...
                respondDialogSignOnOffMultipleTeamsSelectButton.setOnClickListener {
                    for (eachPair in adapter.data) {
                        if (eachPair.first) {
                            SMSSender.sendSMSResponse(context, view, SARResponseCode.SIGN_OFF, dialog, 0, eachPair.second)
                        }
                    }
                }
            }
            ...
            val onePrefixDialogClickListener = DialogInterface.OnClickListener { _, which ->
                if (which == DialogInterface.BUTTON_POSITIVE) {
                    SMSSender.sendSMSResponse(context, view, SARResponseCode.SIGN_OFF, null, 0, teamPrefixArray[0])
                }
            }
            AlertDialog.Builder(context).setTitle(context.getString(R.string.fragment_respond_dialog_sign_off_title))
                .setMessage(context.getString(R.string.fragment_respond_dialog_sign_off_message)).
